...
document.addEventListener('DOMContentLoaded', function () {
  ...
  var rules = {
    ...
    message: {
      ...
      maxLength: 1000,
      message: {
        required: 'ご要望・お問い合わせ内容を入力してください',
  ...
  function showError(field, message) {
    ...
    var existing = field.parentElement.querySelector('.form-error-msg');
  ...
  form.querySelectorAll('input, textarea, select').forEach(function (field) {
    ...
    field.addEventListener(evt, function () {
      ...
      var err = validateField(field.name, isCheck ? field.checked : field.value, isCheck);
  ...
  form.addEventListener('submit', function (e) {
    ...
    if (hasError) {
      ...
      if (firstErr) {
        ...
        var top      = firstErr.getBoundingClientRect().top + window.scrollY - offset - 20;
